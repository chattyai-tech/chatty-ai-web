      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 sm:pb-10 md:pb-12">
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/5 dark:bg-white/5 border border-white/10 dark:border-white/10 backdrop-blur-sm">
            ...
            <span className="text-sm font-medium text-[var(--text-secondary)]">Enterprise AI Platform</span>
          </div>
        </div>
        ...
        <header className="text-center mb-6 sm:mb-8 max-w-4xl mx-auto px-4">
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-[-0.03em] mb-5 sm:mb-6 leading-[1.05]">
            ...
            <span className="bg-gradient-to-r from-violet-400 via-purple-400 to-blue-400 bg-clip-text text-transparent">Enterprise AI</span>
          ...
          <p className="text-xl sm:text-2xl md:text-3xl font-bold mb-5 sm:mb-6 tracking-tight">
            <span className="bg-gradient-to-r from-purple-400 to-violet-400 bg-clip-text text-transparent">Powerful.</span>
            <span className="mx-1.5 sm:mx-2 text-[var(--text-primary)]">Secure.</span>
            <span className="bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">On-Premise.</span>
          </p>
          <p className="text-base sm:text-lg text-[var(--text-secondary)]/90 max-w-2xl mx-auto leading-relaxed">
            Chatty connects to your documents, databases, internal systems, and workflows.
            Ask questions, analyze data, and run AI agents that reason, decide, and act — with full control and enterprise-grade security.
          </p>
          <div className="inline-flex items-center gap-2 sm:gap-3 text-xs sm:text-sm font-semibold tracking-[0.15em] uppercase">
        ...
        <div className="mb-8 sm:mb-12 md:mb-16 w-full relative z-10">
          <div className="relative mx-auto max-w-[1400px] w-full px-4 sm:px-6 lg:px-8 isolate overflow-hidden">
            ...
            <div className="relative overflow-hidden rounded-lg sm:rounded-xl md:rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-card)] shadow-xl w-full isolate" style={{ textAlign: 'left', contain: 'layout style paint' }}>
              ...
              <div className="h-12 sm:h-14 md:h-16 bg-[var(--bg-header)] dark:bg-[#2d1f45] flex items-center justify-between px-3 sm:px-4 md:px-5 lg:px-6 w-full relative z-10 border-b border-gray-700 dark:border-white/10">
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                    <BrandLogo size={16} className="h-6 sm:h-7 md:h-8 w-auto" variant="hero" />
                  </div>
                  ...
                  <div className="relative" ref={modelMenuRef}>
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); setIsModelMenuOpen(!isModelMenuOpen); handleInteraction(); }}
                    ...
                    >
                      <span className="whitespace-nowrap">{getViewLabel(state.currentView)}</span>
                      <ChevronDown size={14} className="text-gray-400 dark:text-gray-200 sm:w-4 sm:h-4" />
                    </button>

                    {/* Dropdown Menu */}
                    {isModelMenuOpen && (
                      <div className="absolute top-full left-0 mt-1.5 w-[220px] bg-[var(--bg-card)] rounded-xl shadow-xl border border-[var(--border-primary)] py-1.5 px-1.5 z-50 animate-fade-in-up origin-top-left" style={{ contain: 'layout style paint' }}>
                        {[
                          ...
                          >
                            ...
                            {state.currentView === item.id && <CheckCircle2 size={14} className="text-[var(--accent-violet)]" />}
                          </button>
                        ))}
                      </div>
                    )}
                ...
                <div className="flex items-center gap-2">
                  {!state.isAutoplay && (
                    <button type="button" onClick={handleReplay} className="p-2 text-gray-400 hover:bg-gray-700 rounded-full transition-colors" title="Replay Demo">
                      <RotateCw size={18} />
                    </button>
                  )}
                  <button type="button" className="p-2 text-gray-400 hover:bg-gray-700 rounded-full transition-colors" aria-label="Share">
                    <Share size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex overflow-hidden w-full relative isolate" style={{ minHeight: 'clamp(400px, 60vh, 700px)', maxHeight: 'clamp(600px, 85vh, 900px)', contain: 'layout style paint', isolation: 'isolate' }}>

                {/* Sidebar - Hidden on mobile, drawer on tablet - Larger */}
                <div className={`${isSidebarOpen ? 'w-[240px] xl:w-[260px]' : 'w-0'} hidden lg:flex flex-col border-r border-[var(--border-primary)] bg-[var(--bg-sidebar)] shrink-0 relative z-0 overflow-hidden transition-all duration-300 ease-in-out`} style={{ willChange: 'width' }}>
                ...
                >
                  <PanelLeftClose size={18} className="text-[var(--text-primary)]" />
                </button>
                ...
                {isSidebarOpen && (
                  <>
                    ...
                    />
                    <div className={`lg:hidden fixed left-0 top-0 bottom-0 w-[280px] bg-[var(--bg-sidebar)] z-50 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                      <Sidebar currentView={state.currentView} onNavigate={(view) => { handleNavigate(view); setIsSidebarOpen(false); }} />
                    </div>
                  </>
                )}

                {/* Main Content */}
                <div className="flex-1 flex flex-col bg-[var(--bg-card)] min-w-0 w-full relative z-0 overflow-hidden" style={{ contain: 'layout style' }}>
                  {renderMainContent()}
                </div>
              </div>
            </div>
