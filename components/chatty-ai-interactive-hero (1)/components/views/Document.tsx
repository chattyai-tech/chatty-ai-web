        ...
        <>
            {parts.map((part, i) => 
                part.toLowerCase() === term.toLowerCase() ? (
                    <span key={i} className="bg-yellow-200 text-slate-900 rounded-[2px] shadow-sm px-0.5 transition-all duration-300">
                        {part}
                    </span>
                ) : (
...
export const DocumentView: React.FC<DocumentProps> = ({ isAutoplay, onInteract }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-white font-sans relative" onClick={onInteract}>
            
            {/* Header */}
            <div className="h-14 border-b border-slate-200 flex items-center justify-between px-6 shrink-0 bg-white transition-all">
                <div className="flex items-center gap-4 flex-1 overflow-hidden">
                    {!isSearchActive ? (
                        <div className="flex items-center gap-2 animate-fade-in whitespace-nowrap">
                            <h2 className="text-lg font-semibold text-slate-800">Company procedures</h2>
                            <span className="text-slate-400 cursor-pointer hover:text-slate-600"><Plus size={16} /></span>
                        </div>
                    ) : (
                        <div className="flex items-center gap-2 flex-1 max-w-md animate-fade-in-up">
                            <div className="relative flex-1">
                                ...
                                <input 
                                    autoFocus
                                    type="text" 
                                    className="w-full bg-slate-100 border border-transparent focus:bg-white focus:border-blue-200 rounded-md py-1.5 pl-9 pr-8 text-sm outline-none transition-all"
                ...
                <div className="flex items-center gap-3 text-slate-400 shrink-0">
                    <button 
                        ...
                        className={`p-2 rounded-md transition-colors ${isSearchActive ? 'bg-blue-50 text-blue-600' : 'hover:bg-slate-50 hover:text-slate-600'}`}
                        title="Toggle Search"
                    >
                        <Search size={18} />
                    </button>
                    ...
                    <div className="h-6 w-[80px]">
                        <div className="font-black text-slate-300 text-lg tracking-widest text-right select-none">LOGO</div>
                    </div>
                </div>
            </div>
                    ...
                    <div key={msg.id} className="animate-fade-in">
                        {msg.type === 'user' ? (
                            <div className="flex justify-end">
                                <div className="bg-slate-50 border border-slate-100 rounded-2xl rounded-tr-sm px-5 py-3 max-w-2xl shadow-sm group">
                                    <div className="text-[11px] text-slate-400 mb-1 text-right">{msg.timestamp}</div>
                                    <p className="text-slate-700 text-[15px]">
                                        <Highlight text={msg.text || ""} term={searchTerm} />
                                    </p>
                                    <div className="flex justify-end gap-2 mt-2 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <PenLine size={12} className="cursor-pointer hover:text-slate-600"/>
                                        <Trash2 size={12} className="cursor-pointer hover:text-slate-600"/>
                                    </div>
                                </div>
                            </div>
                        ) : (
                            <div className="flex gap-4 max-w-4xl">
                                <div className="h-8 w-8 rounded-lg bg-green-900 flex items-center justify-center shrink-0 mt-1 shadow-sm text-white font-bold text-xs">
                                    HR
                                </div>
                                <div className="flex-1 space-y-2">
                                    <div className="flex items-center gap-2">
                                        <span className="font-semibold text-sm text-slate-900">Company procedures</span>
                                        <span className="text-[11px] text-slate-400">{msg.timestamp}</span>
                                    </div>

                                    {/* Render Bot Response with Highlighting */}
                                    {msg.text ? (
                                        <div className="prose prose-sm max-w-none text-slate-700 bg-white">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-3xl px-4 z-10">
                <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-2 flex items-center gap-2 ring-1 ring-slate-900/5 transition-all focus-within:ring-blue-500/20 focus-within:border-blue-300">
                     ...
                     <div className="px-3 py-1 bg-slate-100 rounded text-xs text-slate-500 font-medium flex items-center gap-1 cursor-default">
                         <Sparkles size={12} />
                     ...
                     <input 
                        ...
                        className="flex-1 bg-transparent border-none outline-none text-slate-700 px-2 h-10"
                        value={inputValue}
                     ...
                     <button 
                        onClick={handleSend}
                        className={`p-2 rounded-full transition-colors shadow-md flex items-center justify-center w-9 h-9 ${inputValue.trim() ? 'bg-black text-white hover:bg-slate-800' : 'bg-slate-200 text-slate-400'}`}
                    >
...
const BotResponseContent: React.FC<{ searchTerm?: string }> = ({ searchTerm = "" }) => (
    <div className="prose prose-sm max-w-none text-slate-700">
        ...
        <h3 className="text-[16px] font-bold text-slate-900 mb-2"><Highlight term={searchTerm} text="1. Purpose and Scope" /></h3>
        <p className="mb-4">
        ...
        <ul className="list-disc pl-5 space-y-1 mb-4 marker:text-slate-400">
